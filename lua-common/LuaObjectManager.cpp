...
cn::vimfung::luascriptcore::LuaObjectManager *
cn::vimfung::luascriptcore::LuaObjectManager::SharedInstance() {
...
void cn::vimfung::luascriptcore::LuaObjectManager::removeObject(int objectId) {
    ...
    if (it != _objectPool.end()) {
        it->second->_retainCount--;
        _objectPool.erase(it);
    }
}

cn::vimfung::luascriptcore::LuaObject *
cn::vimfung::luascriptcore::LuaObjectManager::getObject(int objectId) {
    LuaObjectMap::iterator it = _objectPool.find(objectId);
    if (it != _objectPool.end()) {
        return it->second;
    }
