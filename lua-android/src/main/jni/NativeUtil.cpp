 ...
 * Signature: ()Lcom/hacknife/lua/LuaContext;
 */
JNIEXPORT jobject JNICALL Java_com_hacknife_lua_LuaNativeUtil_createContext
        (JNIEnv *env, jclass obj)
{
    LuaContext *context = new LuaContext("android");
    jobject jcontext = JavaEnv::createJavaLuaContext(env, context);
    context -> release();

    //绑定导出原生类型方法
    context->onExportsNativeType(JavaEnv::getExportsNativeTypeHandler());
        ...
        (JNIEnv *env, jclass type, jint contextNativeId, jstring name_, jobject value)
{
    ...
    if (context != NULL)
    {
        ...
        LuaValue *valueObj = JavaConverter::convertToLuaValueByJLuaValue(env, context, value);
        ...
        (JNIEnv *env, jclass type, jint contextNativeId, jstring name_)
{
    ...
    if (context != NULL)
    {
        ...
        retObj = JavaConverter::convertToJavaLuaValueByLuaValue(env, context, value);
        value -> release();
        env -> ReleaseStringUTFChars(name_, nameStr);
    }
...
{
    LuaContext *context = JavaConverter::convertToContextByJLuaContext(env, jcontext);
    if (context != NULL)
    {
        if (enabled)
        {
            //设置异常捕获
            context -> onException(JavaEnv::getExceptionHandler());
        }
        else
        {
            //取消异常捕获
 ...
 * Class:     com_hacknife_lua_LuaNativeUtil
 * Method:    evalScript
 * Signature: (ILjava/lang/String;)Lcom/hacknife/lua/LuaValue;
 */
        ...
        (JNIEnv *env, jclass obj, jint nativeContextId, jstring script, jobject jscriptController)
{
    ...
    if (context != NULL)
    {
        LuaScriptController *scriptController = JavaConverter::convertToScriptControllerByJScriptController(env, jscriptController);
    ...
    if (context != NULL)
    {
        ...
        if (arguments != NULL)
        {
            ...
            for (int i = 0; i < length; ++i)
            {
                ...
                LuaValue *value = JavaConverter::convertToLuaValueByJLuaValue(env, context, item);
                if (value != NULL) {
        ...
        retObj = JavaConverter::convertToJavaLuaValueByLuaValue(env, context, retValue);
        retValue -> release();
        ...
        (JNIEnv *env, jclass thiz, jint nativeContextId, jstring methodName)
{
    ...
    {
        const char *methodNameStr = env -> GetStringUTFChars(methodName, NULL);
        context -> registerMethod(methodNameStr, JavaEnv::luaMethodHandler());
        env -> ReleaseStringUTFChars(methodName, methodNameStr);
    }
}
...
{
    JavaEnv::releaseObject(env, nativeObjectId);
}
        ...
        (JNIEnv *env, jclass thiz, jobject jcontext, jobject func, jobjectArray arguments, jobject jscriptController)
{
    ...
    {
        //获取LuaFunction
        LuaValue *value = JavaConverter::convertToLuaValueByJObject(env, context, func);
        if (value != NULL)
        {
            LuaArgumentList argumentList;
            ...
            if (arguments != NULL)
            {
                ...
                {
                    jobject item = env->GetObjectArrayElement(arguments, i);
                    LuaValue *argValue = JavaConverter::convertToLuaValueByJLuaValue(env, context, item);
                    if (argValue != NULL)
                    {
...
JNIEXPORT void JNICALL Java_com_hacknife_lua_LuaNativeUtil_retainValue(JNIEnv *env, jclass type, jobject jcontext, jobject jvalue)
{
    LuaContext *context = JavaConverter::convertToContextByJLuaContext(env, jcontext);
    if (context != NULL)
    {
        LuaValue *value = JavaConverter::convertToLuaValueByJLuaValue(env, context, jvalue);
        context -> retainValue(value);
        value -> release();
    }
}

JNIEXPORT void JNICALL Java_com_hacknife_lua_LuaNativeUtil_releaseValue(JNIEnv *env, jclass type, jobject jcontext, jobject jvalue)
{
    LuaContext *context = JavaConverter::convertToContextByJLuaContext(env, jcontext);
    if (context != NULL)
    {
        ...
        jobjectArray classMethods)
{
    ...
    if (context != NULL)
    {
        ...
        JavaExportTypeDescriptor *typeDescriptor = new JavaExportTypeDescriptor(typeNameStr, env, type, parentTypeDescriptor);
        ...
        for (int i = 0; i < fieldsLen; ++i)
        {
            ...
            JavaExportPropertyDescriptor *propertyDescriptor = new JavaExportPropertyDescriptor(fieldComps[0], canRead, canWrite);
        ...
        for (int i = 0; i < instanceMethodsLen; ++i)
        {
            ...
            JavaExportMethodDescriptor *methodDescriptor = new JavaExportMethodDescriptor(methodComps[0], methodComps[1], LuaJavaMethodTypeInstance);
        ...
        for (int i = 0; i < classMethodsLen; ++i)
        {
            ...
            JavaExportMethodDescriptor *methodDescriptor = new JavaExportMethodDescriptor(methodComps[0], methodComps[1], LuaJavaMethodTypeStatic);
...
JNIEXPORT void JNICALL Java_com_hacknife_lua_LuaNativeUtil_raiseException(JNIEnv *env, jclass type, jobject jcontext, jstring message)
{
    LuaContext *context = JavaConverter::convertToContextByJLuaContext(env, jcontext);
    if (context != NULL)
    {
...
JNIEXPORT void JNICALL Java_com_hacknife_lua_LuaNativeUtil_runThread(JNIEnv *env, jclass type, jobject jcontext, jobject jhandler, jobjectArray arguments, jobject jscriptController)
{
    LuaContext *context = JavaConverter::convertToContextByJLuaContext(env, jcontext);
    if (context != NULL)
    {
        //获取LuaFunction
        LuaValue *value = JavaConverter::convertToLuaValueByJObject(env, context, jhandler);
        if (value != NULL)
        {
            LuaArgumentList argumentList;
...
JNIEXPORT jobject JNICALL Java_com_hacknife_lua_LuaNativeUtil_createScriptController(JNIEnv *env, jclass type)
{
    LuaScriptController *scriptController = new LuaScriptController();
    jobject jScriptController = JavaEnv::createJavaLuaScriptController(env, scriptController);
    scriptController -> release();

    return jScriptController;
}

JNIEXPORT void JNICALL Java_com_hacknife_lua_LuaNativeUtil_scriptControllerSetTimeout(
        JNIEnv *env,
        jclass type,
        jobject controller,
        jint timeout)
{
    LuaScriptController *scriptController = JavaConverter::convertToScriptControllerByJScriptController(env, controller);
...
JNIEXPORT void JNICALL Java_com_hacknife_lua_LuaNativeUtil_scriptControllerForceExit(
        JNIEnv *env,
        jclass type,
        jobject controller)
{
...
JNIEXPORT jobject JNICALL Java_com_hacknife_lua_LuaNativeUtil_luaValueSetObject(
        JNIEnv *env,
        jclass type,
        jobject jcontext,
        jobject jvalue,
        ...
        jobject jObject)
{
    ...
    if (context != NULL)
    {
        ...
        if (jObject != NULL)
        {
            object = JavaConverter::convertToLuaValueByJLuaValue(env, context, jObject);
        }

        if (value != NULL)
        {
            value -> setObject(keyPathCStr, object);
            retObject = JavaConverter::convertToJavaObjectByLuaValue(env, context, value);
