 ...
 */
static LuaValue *
_luaMethodHandler(LuaContext *context, std::string methodName, LuaArgumentList arguments) {
    ...
    if (jcontext != NULL) {
        if (env->IsSameObject(jcontext, NULL) != JNI_TRUE) {
            ...
            static jmethodID invokeMethodID = env->GetMethodID(contenxtClass, "methodInvoke",
                                                               "(Ljava/lang/String;[Lcom/hacknife/lua/LuaValue;)Lcom/hacknife/lua/LuaValue;");
            ...
            for (LuaArgumentList::iterator it = arguments.begin(); it != arguments.end(); it++) {
                ...
                jobject jArgument = JavaConverter::convertToJavaLuaValueByLuaValue(env, context,
                                                                                   argument);
            ...
            jobject result = env->CallObjectMethod(jcontext, invokeMethodID, jMethodName,
                                                   argumentArr);
            if (result != NULL) {
 ...
 */
static LuaValue *
_luaThreadHandler(LuaContext *context, const std::string &methodName, LuaArgumentList arguments) {
    JNIEnv *env = JavaEnv::getEnv();
    LuaValue *retValue = NULL;
...
static void _luaExceptionHandler(LuaContext *context, std::string const &message) {
    ...
    if (jcontext != NULL) {
        ...
        jfieldID exceptHandlerFieldId = env->GetFieldID(contextCls, "_exceptionHandler",
                                                        "Lcom/hacknife/lua/LuaExceptionHandler;");
        ...
        if (exceptHandler != NULL) {
            ...
            jmethodID onExceptMethodId = env->GetMethodID(exceptHandlerCls, "onException",
                                                          "(Ljava/lang/String;)V");
...
static void _luaExportsNativeTypeHandler(LuaContext *context, std::string const &typeName) {
    ...
    if (jcontext != NULL) {
        static jclass contenxtClass = JavaType::contextClass(env);
        static jmethodID invokeMethodID = env->GetMethodID(contenxtClass, "exportsNativeType",
                                                           "(Ljava/lang/String;)V");
...
void JavaEnv::associcateInstance(JNIEnv *env, jobject instance,
                                 cn::vimfung::luascriptcore::LuaObjectDescriptor *descriptor) {
    if (env->IsInstanceOf(instance, JavaType::luaBaseObjectClass(env)) == JNI_TRUE) {
        ...
        std::map<int, LuaObjectDescriptor *>::iterator it = _instanceMap.find(
                descriptor->objectId());
...
LuaObjectDescriptor *JavaEnv::getAssociateInstanceRef(JNIEnv *env, jobject instance) {
    ...
    if (instance != NULL &&
        env->IsInstanceOf(instance, JavaType::luaBaseObjectClass(env)) == JNI_TRUE) {
        jclass instanceCls = env->GetObjectClass(instance);
...
std::string JavaEnv::getJavaClassNameByInstance(JNIEnv *env, jobject instance) {
    ...
    if (env->IsInstanceOf(instance, JavaType::luaObjectClass(env)) == JNI_TRUE) {
        ...
        jmethodID getModuleNameMethodId = env->GetStaticMethodID(JavaType::moduleClass(env),
                                                                 "_getModuleName",
                                                                 "(Ljava/lang/Class;)Ljava/lang/String;");
        jstring jclassName = (jstring) env->CallStaticObjectMethod(JavaType::moduleClass(env),
                                                                   getModuleNameMethodId,
                                                                   instanceCls);
        if (jclassName != NULL) {
...
jobject JavaEnv::getExportTypeManager(JNIEnv *env) {
    ...
    if (exportTypeManager == NULL) {
        ...
        jmethodID defaultManagerMethodId = env->GetStaticMethodID(exportTypeManagerCls,
                                                                  "getDefaultManager",
                                                                  "()Lcom/hacknife/lua/LuaExportTypeManager;");
